...
test('should use a default `start` of `0` and a default `end` of `length`', (t) => {
  t.deepEqual([...fill(array, 'a')], ['a', 'a', 'a'])
})

test('should work with a positive `start`', (t) => {
  t.deepEqual([...fill(array, 'a', 1)], [1, 'a', 'a'])
})

test('should work with a `start` >= `length`', (t) => {
...
test('should work with `start` >= `end`', (t) => {
  t.deepEqual([...fill(array, 'a', 2, 2)], [1, 2, 3])
  t.deepEqual([...fill(array, 'a', 3, 2)], [1, 2, 3])
})

test('should work with a positive `end`', (t) => {
  t.deepEqual([...fill(array, 'a', 0, 1)], ['a', 2, 3])
})

test('should work with a `end` >= `length`', (t) => {
