...
test('should be an array-like object#should have a length property', (t) => {
  t.is(range(10).length, 10)
  t.is(range(10, 15).length, 5)
  t.is(range(10, 20, 3).length, 4)
})

test('should be an array-like object#should work as a parameter to Array.from', (t) => {
  t.notThrows(() => Array.from(range(10)))
})

test('should adhere to the iterator protocol', (t) => {
  const r = range(10, 13)
...
test('should be indexable from the end', (t) => {
  t.is(range(10)[-2], 8)
  t.is(range(10, 15)[-2], 13)
  t.is(range(10, 20, 3)[-2], 16)
})

test('should not be indexable from the end when the range is infinite', (t) => {
  t.is(range(Number.POSITIVE_INFINITY)[-1e3], undefined)
  t.is(range(Number.NEGATIVE_INFINITY)[-1e3], undefined)
})

test('should be reversible', (t) => {
